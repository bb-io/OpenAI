...
public class ChatActions : BaseActions
{
    ...
    [Action("Perform LQA Analysis", Description = "Perform an LQA Analysis of the translation. The result will contain " +
                                                  "a text with issues if any.")]
        ...
        [ActionParameter] GetTranslationIssuesRequest input)
    {
        ...
        var userPrompt = $"{(input.SourceLanguage != null ? $"The {input.SourceLanguage} " : "")}\"{input.SourceText}\" was " +
                         $"translated as \"{input.TargetText}\"{(input.TargetLanguage != null ? $" into {input.TargetLanguage}" : "")}";
